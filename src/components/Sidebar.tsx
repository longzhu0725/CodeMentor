    ...
    <aside className="flex h-full w-[68px] flex-col items-center border-r border-border bg-card py-4 lg:w-60 lg:items-stretch lg:px-3">
      ...
      <div className="flex items-center gap-2.5 px-1 lg:px-2">
        ...
        <div className="hidden lg:block">
          ...
          <div className="text-[10px] leading-tight text-muted">AI 算法导师</div>
        </div>
      </div>
      ...
      <button
        ...
        className="mt-4 mx-1 lg:mx-0 flex items-center justify-center gap-2 rounded-xl border border-accent/30 bg-accent/10 px-3 py-2 text-sm font-medium text-accent transition-colors hover:bg-accent/20 lg:justify-start"
      >
      ...
      <div className="hidden lg:block mt-3 flex-1 overflow-y-auto">
        <button
          onClick={() => setShowSessions(!showSessions)}
          className="flex w-full items-center justify-between px-2 py-1.5 text-[11px] font-medium text-muted hover:text-foreground"
        >
          <span>历史会话</span>
          <svg
            width="12"
          ...
          <div className="mt-1 space-y-0.5">
            {sessions.slice(0, 20).map((s) => (
              ...
              >
                ...
                {s.id === activeSessionId && sessions.length > 1 && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
      ...
      {/* Navigation */}
      <nav className={`flex w-full flex-col gap-1.5 ${showSessions ? 'mt-2' : 'mt-7 lg:mt-auto'}`}>
        {NAV_ITEMS.map((item) => {
      ...
      {/* Streak */}
      <div className="mb-2 mt-2 w-full px-1 lg:px-2">
        <div className="flex items-center justify-center gap-2 rounded-xl border border-warning/30 bg-warning/10 px-3 py-2.5 lg:justify-start">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-warning">
          ...
          <span className="lg:hidden text-xs font-bold text-warning">{streak}</span>
        </div>
      </div>
