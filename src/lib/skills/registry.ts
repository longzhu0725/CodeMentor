...
- 错题本与薄弱点分析`,
  ...
  {
    name: 'problem-quality-check',
    description:
      '题目质量保障方法论。当生成新题目时触发。确保题目的测试用例正确、结构完整、难度合理。不依赖AI判断测试用例正确性，而是用确定性验证。',
    triggerKeywords: ['验证题目', '检查题目', '题目质量', '测试用例验证'],
    triggerCommands: ['/validate'],
    content: `# 题目质量保障方法论

## 核心原则
题目的正确性不能依赖AI判断，必须用确定性方法验证。

## 验证流程（3 阶段）

### 阶段 1：结构验证（静态，无需运行代码）
检查以下字段是否齐全且格式正确：
- id: 小写字母+数字+短横线，如 two-sum
- title: 非空中文标题
- topicId: 必须在知识库 15 个知识点中
- difficulty: 整数 1-5
- description: 清晰无歧义的题目描述
- examples: 至少 1 个示例，包含 input 和 output
- constraints: 至少 1 条约束
- starterCode: 必须包含 def 函数定义
- hints: 至少 3 条分级提示（由浅入深）
- solution: 参考解答，包含 def 函数定义
- testCases: 至少 3 个测试用例，每个包含 input 和 expectedOutput
- timeComplexity: 大O表示法
- spaceComplexity: 大O表示法

### 阶段 2：测试用例验证（动态，用 Pyodide 运行）
1. 将参考解答代码加载到 Pyodide
2. 提取函数名（从 def 语句）
3. 对每个测试用例：
   - 解析输入（支持 "nums = [2,7], target = 9" 格式）
   - 调用函数执行
   - 比较实际输出与期望输出
4. 所有测试用例必须全部通过，否则题目有误
...
## 测试用例设计要求
1. **正常用例**：典型输入，验证基本功能
2. **边界用例**：空数组、单元素、最大/最小值
3. **异常用例**：负数、零、重复元素
4. **性能用例**（可选）：大数据量测试

## 常见问题防范
- 测试用例的 expectedOutput 必须与参考解答的实际输出一致
- 输入格式必须与 starterCode 的函数签名匹配
...
## 失败处理
- 结构验证失败：补充缺失字段后重新验证
- 测试用例验证失败：修正 expectedOutput 或参考解答
- 多次验证失败：降级到本地题库随机选题`,
  },
];
